...
export default function FoodCard({ product }) {
  ...
  return (
    <Link to={`/products/${_id}`} className="group block">
      <div className="surface overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-raised">
        ...
        <div className="px-3.5 py-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-1.5">
              <span className="mono text-[17px] font-bold t-text-1 tracking-tight">₹{currentPrice}</span>
              <span className="mono text-xs t-text-4 line-through">₹{originalPrice}</span>
            </div>
            <div className="text-right">
              <span className="mono text-[11px] t-text-4">{stock} left</span>
              {!urgency && (
